...
struct GalleryEditingView: View {
        ...
        Group {
            if let draft = presenter.draft {
                GalleryEditorPreviewSurface(
                    ...
                    onResizeEnded: presenter.commitCropResize
                )
            } else {
                AppColours.appColor
            }
    ...
    private var toolContent: some View {
            ...
            VStack(spacing: 12) {
                    ...
                    LazyHStack(spacing: 16) {
                        ForEach(presenter.filters) { filter in
                            FilterChipView(
                                ...
                                action: { presenter.selectFilter(filter.id) }
                            )
                        }
                    }
                    .padding(.horizontal, 16)
                ...
                if presenter.draft?.selectedFilterID != GalleryFilter.original.id {
                    FilterIntensitySlider(
                        ...
                        onChangeEnded: presenter.commitFilterIntensity
                    )
                    .padding(.horizontal, 16)
                }
            }
        case .adjustments:
            if let adjustments = presenter.draft?.adjustments {
                GalleryAdjustmentControlsView(
                    ...
                    onChangeEnded: presenter.commitAdjustment,
                    onToggleMonochrome: presenter.toggleMonochrome
                )
            }
        case .crop:
        ...
        case .rotate:
            HStack(spacing: 32) {
                Button { presenter.rotateDraft(by: -90) } label: {
